...
if [ $javascript_files_count -lt $clojure_files_count ] &&
   [ $ruby_files_count       -lt $clojure_files_count ]; then
  tmux-ide-clojure "$*"
elif [ $clojure_files_count -lt $javascript_files_count ] &&
     [ $ruby_files_count    -lt $javascript_files_count ]; then
  tmux-ide-javascript "$*"
elif [ $clojure_files_count    -lt $ruby_files_count ] &&
     [ $javascript_files_count -lt $ruby_files_count ]; then
  tmux-ide-ruby "$*"
else
